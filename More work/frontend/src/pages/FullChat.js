...
const FullChat = ({ messages, isGroupChat }) => {
  const { user } = ChatState(); // Get user from ChatState
  const messagesEndRef = useRef(null); // Create a reference for the end of the messages
      ...
      {messages &&
        messages.map((m, i) => (
          <div
            style={{
              display: "flex",
              flexDirection: "column", // Arrange sender name and message vertically
              alignItems:
                m.sender && m.sender._id === user?._id
                  ? "flex-end"
                  : "flex-start",
            ...
            key={m._id || i} // Fallback to index if _id is missing
          >
            {/* Show sender name if the message is not sent by the user */}
